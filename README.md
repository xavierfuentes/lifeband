...
`lifeBand` is designed to make life easy by providing a basic framework
...
lifeBand also comes prepackaged with the most popular design frameworks
...
The principal goal of `lifeBand` is to set projects up for long-term
success.  So `lifeBand` tries to follow best practices everywhere it can.
These are:
...
- It should be responsive to evidence. Community feedback is therefore crucial
  to the success of `lifeBand`.

But `lifeBand` is not an example of an AngularJS app: this is a
...
getting `lifeBand` up and running. Though `lifeBand` is really simple
...
`lifeBand` uses [Grunt](http://gruntjs.org) as its build system, so
...
There are many Bower packages used by `lifeBand`, like Twitter Bootstrap
...
Technically, `lifeBand` is now ready to go.
...
`lifeBand`. What follows in this section is a quick introduction to the
...
`lifeBand` also includes [Live Reload](http://livereload.com/), so you no
...
default with `lifeBand`) you need to tell Live Reload to allow it. Go to
