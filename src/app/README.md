...
As `lifeBand` is quite minimal, take a look at the two provided submodules
...
.controller( 'AppCtrl', function AppCtrl ( $scope, $location ) {
  $scope.$on('$stateChangeSuccess', function(event, toState, toParams, fromState, fromParams){
    if ( angular.isDefined( toState.data.pageTitle ) ) {
      $scope.pageTitle = toState.data.pageTitle + ' | lifeBand' ;
    }
  });
})
```
...
One of the design philosophies of `lifeBand` is that tests should exist
