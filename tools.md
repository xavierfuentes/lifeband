# The Tools Used in `lifeBand`

## Introduction

`lifeBand` is standards-based, so it uses all the usual tools to manage
...
project makes use and how they fit in to the `lifeBand` picture.

## Git

[Git](http://git-scm.com/) is a distributed version control system.
`lifeBand` uses git to manage its codebase. While in theory you don't have
to use Git once you download `lifeBand`, this project makes the assumption
...
While `lifeBand` makes heavy use of Node.js behind the scenes, you as the
...
you first start with `lifeBand` you have to tell NPM to install the
...
Don't worry about knowing Node.js in order to use `lifeBand`; Grunt is
...
to get started with `lifeBand`. But for those curious or looking to go a
...
Grunt is the engine behind `lifeBand`. It's the magic that makes it move.
...
`lifeBand` comes with a `bower.json` file that looks something like this:
...
in `lifeBand` by default in there to see what I mean.
